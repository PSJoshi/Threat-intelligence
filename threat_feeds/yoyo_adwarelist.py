...
class Yoyo_adware(Threat_intelligence_base):
    ...
    def __init__(self,feed_url='',proxy_enabled='no',proxy_host=None,proxy_port=None,proxy_user=None,proxy_password=None,proxy_auth=None):
        super(Yoyo_adware, self).__init__(feed_url,proxy_enabled,proxy_host,proxy_port,proxy_user,proxy_password,proxy_auth)
    ...
    def process_feed(self, feed_outputs=None):
        try:
            ...
            if self.feed_data:
                ...
                # ignore top/bottom entries as it contains generic information
                self.feed_data = self.feed_data[33:-7]				
                self.feed_data = [ item for item in self.feed_data if item != ''] 
                self.feed_data = [ item.split(' ')[1].strip() for item in self.feed_data] 
    ...
    def _write_data(self,feed_details):
        ...
        try:
            ...
            # variables required for generation of Bro CSV  
